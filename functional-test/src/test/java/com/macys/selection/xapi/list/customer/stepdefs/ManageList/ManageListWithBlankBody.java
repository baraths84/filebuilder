...
public class ManageListWithBlankBody {
	  private Response response;
	  private ValidatableResponse json;
	  private RequestSpecification request;

	  @Given("^a customer with listGuid \"([^\"]*)\" and with userId \"([^\"]*)\"$")
	  public void a_customer_tries_to_update_list_with_blank_body(String listguid, String userid) {
		  request = given().pathParam("listGuid", listguid).queryParam("userId", userid);
	  }

	  @When("^customer tries to manage the list with blank body$")
	  public void customer_manage_a_list_with_blank_body() {
	    String manageList = "";
	    response = request
	  ...
	  @Then("^the manage list with blank body status code is \"([^\"]*)\"$")
