...
public class MergeListOfCustomerWihtoutUserGuid {
    private static final Logger LOGGER = LoggerFactory.getLogger(MergeListOfCustomerWihtoutUserGuid.class);
    ...
    public void a_customer_without_userGuid_merge_list_with_guest() {
        request = given();
    }

    @When("^list of customer without userGuid and guest list are merged$")
    public void customers_without_userGuid_and_guest_lists_are_merged() throws IOException {
        String merge = "{\"wishListRequestDTO\":{\"guestUserId\":80020002,\"userId\":80020003}}";
        response = request
    ...
    @Then("^merge lists of customer without userGuid status code is \"([^\"]*)\"$")
