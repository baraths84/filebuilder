 ...
 **/
public class GetOwnerListsWithHideCollabTrueFlag {

    private Response response;
    private ValidatableResponse json;
    private RequestSpecification request;

    @Given("^user with userGuid \"([^\"]*)\" who has owners and collaborative lists and wants to see only owner lists$")
    public void a_customer_with_userGuid_want_to_see_only_ownre_lists(String userGuid) {
        request = given().queryParam("userGuid", userGuid).queryParam("hideCollab", true);
        System.out.println("Customer lists request" + request);
    }

    @When("^user gets only owner lists$")
    public void a_customer_retrieves_only_owner_lists_data() {
        response = request
    ...
    @Then("^get owner lists only data by userGuid status code is \"([^\"]*)\"$")
    public void verify_status_code(int statusCode) {
        json = response.then().statusCode(statusCode);
    }

    @And("^response contains only owner lists data$")
    public void contains_only_onwer_lists_data() {
        json.body("listsPresentation.lists.listCollaborator", equalTo(null));
