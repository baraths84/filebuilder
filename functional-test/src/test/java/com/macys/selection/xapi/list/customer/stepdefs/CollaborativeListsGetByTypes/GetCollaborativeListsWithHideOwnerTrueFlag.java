 ...
 **/
public class GetCollaborativeListsWithHideOwnerTrueFlag {

    private Response response;
    private ValidatableResponse json;
    private RequestSpecification request;

    @Given("^user with userGuid \"([^\"]*)\" who has collaborative and owners lists and wants to see only collaborative list$")
    public void a_customer_with_userGuid_want_to_see_only_collaborative_lists(String userGuid) {
        request = given().queryParam("userGuid", userGuid).queryParam("hideOwner", true);
        System.out.println("Customer lists request" + request);
    }

    @When("^user gets only collaborative lists$")
    public void a_customer_retrieves_all_lists_data_with_hideOwner_true_flag() {
        response = request
    ...
    @Then("^get collaborative lists data by userGuid status code is \"([^\"]*)\"$")
    public void verify_status_code(int statusCode) {
        json = response.then().statusCode(statusCode);
    }

    @And("^response contains collaborative lists data$")
    public void contains_all_lists_data() {
        json.body("listsPresentation.lists.listOwner", equalTo(null));
