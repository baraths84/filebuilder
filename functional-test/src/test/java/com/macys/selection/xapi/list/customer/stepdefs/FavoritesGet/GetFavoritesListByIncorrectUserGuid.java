...
public class GetFavoritesListByIncorrectUserGuid {
	  private Response response;
	  private ValidatableResponse json;
	  private RequestSpecification request;

	  @Given("^a customer with incorrect userGuid \"([^\"]*)\"$")
      public void customer_gets_favorites_list_by_incorrect_userGuid(String userGuid) {
          request = given().pathParam("userguid", userGuid);
      }

	  @When("^customer receives list of favorites by incorrect userGuid$")
	  public void customer_gets_favorites_by_incorrect_userguid() {
          response = request
	  ...
	  @Then("^status code of getting list by incorrect userGuid is \"([^\"]*)\"$")
