 ...
 **/
public class OwnerGetCollaboratorsExceptOne {

    private Response response;
    private ValidatableResponse json;
    private RequestSpecification request;

    @Given("^owner with userGuid \"([^\"]*)\" who has list with several collaborators$")
    public void owner_user_with_list_with_collaborators(String userGuid) {
        request = given().queryParams("userGuid", userGuid, "exclude", "hrn6kdn-100010018");
        System.out.println("Customer lists request" + request);
    }

    @When("^owner gets collaborators data except one from lists$")
    public void owner_retrives_collaborators_except_one_from_list() {
        response = request
    ...
    @Then("^get collaborators data except one from lists status code is \"([^\"]*)\"$")
